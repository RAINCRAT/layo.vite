...
15. **验证方式：非必要不调用浏览器验证**：代码改动默认通过静态分析（`GetDiagnostics`/`Grep`）与 `npm run build` 确认；浏览器验证（`browser_use` 等）成本高，仅限确实需要确认渲染结果/视觉效果的改动（如布局、动画、样式视觉验收）且无法从代码静态判定的情况下才使用，避免每次改动都跑浏览器验证。
