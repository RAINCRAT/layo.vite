...
16. **Element Plus 仅限工单系统且按需加载**：
    ...
    - 详情页正文键值表（Markdown 表格，`字段名 | 值 | 字段名 | 值` 布局）：**列高亮为开关态**——默认所有表格保持 VitePress 默认样式（首行深蓝表头 + 斑马纹），由 `theme/ticket-kv-table.js`（config.js `markdown.config` 注册）在渲染时识别键值表（列数为偶数且表头奇数列为 ≤8 字符短字段名），命中才给 `<table>` 加 `ticket-kv` 类开启优化：字段名列（第 1、3 列）标签化淡底高亮、值列正常显示、首行不做整行深蓝表头、无斑马纹/整行 hover、空单元格行透明（作分组间距）。样式在 `style.css` "组件细节"区块的 `.is-tickets-page .vp-doc table.ticket-kv` 规则，以 `.is-tickets-page` + `.ticket-kv` 双重限定，不影响普通文档表格；想强制关闭可调整插件识别规则。
