...
│       ├── ak-inline.js         # 全站内容标记 markdown-it 插件：[[色:文字]] 彩色强调 + ||文字|| 色块悬停（config.js 与 TicketHeader.vue 共用，见约定 16）
│       ├── ticket-kv-table.js   # 工单键值表列高亮开关插件：识别「字段名 | 值」布局表格自动加 ticket-kv 类（config.js markdown.config 注册，见约定 16）
│       ├── ticket-kv-stack.js   # 工单键值表窄容器堆叠：正文键值表与头部元信息表（el-descriptions）在容器宽度不足时由两对列改为一对列（Layout.vue 挂载，见约定 16）
│       ├── loader.js            # 首屏加载遮罩独立内联脚本生成器（config.js head 注入，随 HTML 解析执行，不依赖主 JS 与主题 CSS）
│       ├── post-date.js         # 文章发布日期派生工具（**已弃用**：日期改为 frontmatter 手动硬编，git 派生已屏蔽，文件保留但未被任何模块引用，勿重新引入，见约定 15）
...
11. **`theme/style.css` 区块约定**（新增样式按对应区块归类追加，不另起炉灶）：
    ...
    - 文档风格化（导航/侧边栏/代码块/表格/引用/滚动条）："组件细节"区块；分隔线 `vp-doc hr` 对齐 `.ak-divider` 模块（两段式粗杠）；页脚 "Last updated" 显示统一隐藏（`.VPDocFooter .last-updated { display: none }`，`lastUpdated: true` 仅作 sitemap lastmod / JSON-LD dateModified 的 SEO 数据源，见约定 14）。内容标记（`theme/ak-inline.js` 的 `[[色:文字]]`/`||文字||`）样式也在此区块：`[[色:文字]]` 为行内代码同款强调块（等宽字 + 1px 描边 + 淡色底 + 辉光），描边/底色/辉光/**文字颜色均随色变**，内容按行内 Markdown 再解析（可内嵌 **粗体**、_斜体_——按各自默认色渲染，`.ak-hl strong/em` 拉回 `--vp-c-text-1`；`代码` 随强调色，`.ak-hl code` 继承块文字色；也可再嵌 `[[色:文字]]` 嵌套强调，插件按平衡扫描配对 `]]`，**嵌套强调去除块外观、仅文字随色变**——`.ak-hl .ak-hl` 清 padding/border/底色/辉光，用于给块内文字单独着色），强调色名须完全遵循 ak-ui 官方调色板 `--ak-color-*`（14 色：white/black/low/basic/primary/secondary/advanced/accent/blue/yellow/dark-blue/light-blue/gray/dark，见约定 12），`.ak-hl-<色名>` 类直接消费同名 `--ak-color-<色名>`（无中间变量、无中→英映射）；`||文字||`（`.ak-spoiler`）同为行内代码同款代码 token（等宽字 + 1px 描边 + 淡色底 + 辉光），默认遮字、hover 显字（明暗双主题经 color-mix 自动适配，无单独 dark 覆盖），`--hl-tint` 仅作其底色透明度旋钮。
    - ak-ui 组件品牌化覆盖："A. 表面分层与按钮/卡片契约"区块（对齐官方 `.ak-button`/`.ak-card` 编译产物：方块几何、50px 高、hover 提亮+浮起、active 压暗遮罩；`.VPButton`/`.VPFeature`/`.vpb-card` 覆盖），其后跟随 B（背景装饰，已移除项说明）/C（交互动效）/D（焦点）/F（导航更多菜单）/E（404 红主题）/G（工单 EP 方舟化）/H（加载遮罩）等区块。真实 `.ak-*` 类的品牌字面量覆盖（如 `.ak-button--action` 官方 #2bf → 品牌主蓝）也集中在本区块。A 区块内还有"VPB 博客组件残留默认样式 ak 化"子区块：vpb 主题样式表加载顺序靠后，其默认圆角（`rounded-*` 工具类）、代码块/行内代码圆角、`:root` 的 `--vp-c-brand` 绿色系变量与 Neue Haas/Iowan 字体都会压过顶部品牌映射——用 `html` 前缀 + `!important`（及 `html:root` 变量重声明）统一拉回 ak 直角几何与品牌蓝（`.vpb-page`/`.vpb-soft-panel`/`.vpb-results-panel`/`.vpb-card--static`/`.vpb-pill`/`.vpb-chip`/头像圆角归零、元信息等宽、文章标题命令字面 900 等）。同区块还含**布局微调**约定：标题行高中文舒适区间（`.vpb-display-title`/`.vpb-page-title` 1.14、`.vp-doc h1` 1.18、`.vpb-card h2` 1.32，均覆盖 vpb 默认 0.98-1.06）；正文/文档 H1 上下间距均衡（`.vp-doc h1` 上 2.5rem 下 1.25rem，`h1 + h2` 上距收紧为 1.5rem——默认 h1 无 margin、h2 自带 48px 上距，导致 H1 上方贴紧前块、下方与 h2 之间过宽）；文章卡片大标题宽度放宽（`.vpb-card h2` 的 `max-width` 18ch → 28ch，vpb 默认限制太窄致中文标题频繁断行；`margin-bottom` 1rem → 1.15rem 缓解多行标题与摘录拥挤）；首页特性卡整体优化（`.VPFeature .box` padding 28px、`.VPFeature .title` 命令字面 900/行高 1.4/字距归零、`.VPFeature .details` 行高 1.7）；竖屏（`@media (max-width: 1279.98px)`）文章页作者块弱化（作者块不在 `.vp-doc` 内，位于 `.content-container`/`.VPDocAside` 下，选择器须用 `html dl.pb-10` 兜底：padding/头像/字号收窄）；卡片高光与面板阴影收敛（`--vpb-panel-shadow`/`--vpb-panel-shadow-hover` 收敛为 0 8px 24px / 0 16px 36px）；`.vpb-results-panel` 背景品牌化为 `--vpb-bg-soft`。
...
14. **SEO/GEO 无硬链接**：
    ...
    - 页面标题：内容页 `<title>` 统一为「页面标题 | 标题后缀」——站点级 `titleTemplate: ':title | ${seo.titleSuffix}'`（后缀来自 `VITE_SEO_TITLE_SUFFIX`，未设置回退 SEO 站名）；`seo.js` 的 `og:title`/`twitter:title`/JSON-LD `headline` 与 `<title>` 使用同一 `titleSuffix`，保证一致；首页（`index.md`）frontmatter 置 `titleTemplate: false` 不追加后缀，并在 frontmatter 自定义描述性 `title`（纯站名过短会触发搜索引擎「标题太短」提示，Bing 建议 50-60 字符）。**frontmatter 显式声明 `og:title`/`og:description` 时优先采用这两个值作为基底**（不回退页面 description）——`og:title`/`og:description`/`twitter:title`/`twitter:description`/JSON-LD `headline`/`description` 同步跟随；**og:title 后缀始终保留**：基底（`og:title` 或页面 title）不含站点名时一律拼「| 标题后缀」（与 `<title>` 模板一致），保证品牌「泠域存储」后缀恒定出现（`seo.js` 的 `buildPageHeadTags` 统一实现，新增页面无需特殊处理）。
    ...
    - 品牌身份：域名注册域 `raincrat.top`（RAINCRAT雨绘巷）与品牌「LAYOSERVE泠域存储」不一致，Bing 按域名归并站点身份时易在结果中显示父域而非品牌。JSON-LD `WebSite`/`Organization` 的 `alternateName` 集中从 `.env` 的 `VITE_SEO_ALT_NAMES` 读取（建议含中文品牌/英文品牌/简称），辅助搜索引擎把品牌词与本站身份关联；新增品牌别名改该变量即可，`seo.js` 无需改动。
...
15. **VPB 博客主题约定**：
    ...
    - 文章发布日期手动硬编：**在 md frontmatter 手写 `date`**（ISO 格式，如 `date: 2026-08-01T00:00:00Z`），已**屏蔽 git 自动派生**——`config.js` 不再调用 `ensurePostDates`/`firstCommitDate`，`transformPageData` 只保留 `processData` 打标（`theme/post-date.js` 文件保留但未被引用，勿重新引入，见结构注释）。VPB 的 `posts.data.js`（content loader）直接读 md 原始 frontmatter，列表/文章页日期与 JSON-LD `datePublished` 均取 frontmatter `date`；新增文章必须手写 `date`，缺省时文章不显示日期（构建不报错）。
...
16. **Element Plus 仅限工单系统且按需加载**：
    ...
    - 数据：时间线数据来自工单 md 的 frontmatter `updates`（按时间倒序，详情页以时间线展示，首条即最新状态；不单独展示"更新时间"，不读 frontmatter `updatedAt`）；**工单不参与任何 date 自动生成**——`createdAt` 与 `updates` 内的时间戳均由 frontmatter 手动硬编，页面 `lastUpdated`（git 派生）仅作 sitemap lastmod 数据源且工单页已被 SEO `dirs` 整目录排除；时间线仅展开最近 7 条，超过 7 条时面板内部上下滚动查看其余（滚动上限由 `TicketHeader.vue` 在 `onMounted` 实测前 7 条高度后动态设置 `max-height`，非固定值）；时间戳置于内容左侧（`.el-timeline-item__wrapper` flex 两列 + 时间戳固定列宽、加大字号、终端等宽字体栈 `--ak-font-pixel`，设备自带无需登记许可）；终态工单（status 为 已完成/已关闭）时间轴最新一项取消黄色高亮与呼吸脉冲，与先前项统一为 ak 蓝（`.is-done` 类，活跃工单保持高亮）；`channel`（渠道）为自定义字段，列表标签配色见 `Ticket.vue` 的 `channelTag`（未匹配值回退默认色）；模板文件 `assets/tickets/example.md` 由 `tickets.data.js` 排除。时间线 `text` 支持行内 Markdown（**粗体**、_斜体_、`行内代码`、`[[色:文字]]` 彩色强调（色名取 ak-ui 官方调色板 14 色，见约定 11）、`||文字||` 色块悬停）——由 `theme/ak-inline.js` 插件渲染（`TicketHeader.vue` 的独立 MarkdownIt 与全站 VitePress Markdown 共用同一插件，语法一致；其自定义 text 规则把 `|` 加入终结字符，`||x||` 才能出现在文本中间），样式在 `style.css` 的"内容标记"区块（`.ak-hl-<色名>`/`.ak-spoiler`，见约定 11/12）。
    - **不要**在 `theme/index.js` 全局注册 Element Plus，避免全站包体与样式污染。
    - 详情页正文键值表（Markdown 表格，`字段名 | 值 | 字段名 | 值` 布局）：**列高亮为开关态**——默认所有表格保持 VitePress 默认样式（首行深蓝表头 + 斑马纹），由 `theme/ticket-kv-table.js`（config.js `markdown.config` 注册）在渲染时识别键值表（列数为偶数且表头奇数列为 ≤8 字符短字段名），命中才给 `<table>` 加 `ticket-kv` 类开启优化：字段名列（第 1、3 列）标签化淡底高亮、值列正常显示、首行不做整行深蓝表头、无斑马纹/整行 hover、空单元格行透明（作分组间距）。样式在 `style.css` "组件细节"区块的 `.is-tickets-page .vp-doc table.ticket-kv` 规则，以 `.is-tickets-page` + `.ticket-kv` 双重限定，不影响普通文档表格；想强制关闭可调整插件识别规则。**窄容器堆叠**：容器宽度不足以并排容纳整表时，`theme/ticket-kv-stack.js`（Layout.vue 挂载，内部自管理 onMounted/resize/fonts.ready/路由切换监听）给 `<table>` 加 `is-kv-stacked` 类，整表由两对列改为一对列（每行「字段名|值」上下两对，CSS grid 自动重排，DOM 顺序不变故 nth-child 列样式自动保持）；判定带 24px 冗余提前量，临界宽度即整体堆叠，避免字体加载后值列补挤换行。**头部元信息表同套机制**：详情页头部 `el-descriptions`（`.ticket-header__meta`）固定 `:column="2"`（每行「字段名|值|字段名|值」两对列，与正文键值表一致），同一扫描器按相同判定加 `is-meta-stacked` 类，由 grid 把每行 4 单元格重排为两行两列（样式在 `style.css` G7 区块，`.is-meta-measuring` 测量态使表格按 max-content 渲染）。
